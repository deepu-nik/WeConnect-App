...
export default function GroupChatScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        unsubscribeGroup = (await import('../services/groupService')).subscribeToGroups; // keep module hot-reload friendly
    ...
    load();
    const start = async () => {
      unsubscribeMessages = subscribeToGroupMessages(groupId, setMessages, (error) => console.error('Group messages failed:', error));
    };
    start();
    const interval = setInterval(async () => {
      const latest = await getGroup(groupId).catch(() => null);
      if (latest) setGroup(latest);
    }, 1500);
    return () => { if (typeof unsubscribeMessages === 'function') unsubscribeMessages(); clearInterval(interval); if (typeof unsubscribeGroup === 'function') unsubscribeGroup(); };
  }, [groupId]);
  ...
  const saveName = () => {
    Alert.prompt?.('Rename group', 'Enter a new name', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Save', onPress: (value) => value?.trim() && updateGroup(groupId, { name: value.trim() }).catch((e) => Alert.alert('Could not rename', e.message)) },
    ]);
  };
